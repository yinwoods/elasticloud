# coding:utf-8
import urllib, logging, json, time, httplib
from django.conf import settings
from rest_util import simple_post
from config_util import get_docker_proxy_url
from config_util import get_network_name
from config_util import get_zk_hosts
from zk_util import get_min_cluster
from web.models.res import Compute
from rest_util import isOnService, az_upload, az_post, simple_get
...
def boot_storage_hdfs_master(USER_ID, memory):
    ...
    return simple_post(url + "/ws/containers/create", params)
    ...
    if min_cluster is None:
        ...
        if price > cost:
            ...
            if on_line:
                ...
                if az_result:
                    execute = Execute(user=job.user, job=job, c_ip=min_cluster['ip'], s_ip=storage.m_ip, execute_log='')
                    execute.save()
...
def launch_by_new(storage, user_id, az_port, job):
    ...
    if success:
        ...
        if on_line:
            ...
            if az_result:
                execute = Execute(user=job.user, job=job, c_ip=ip, s_ip=storage.m_ip, execute_log='')
                execute.save()
                return 'Job has been submited successfully!'
            else:
...
def az_submit(address, job):
    try:
        ...
        session = json.loads(az_post(address, az_port, az_timeout, '/', params))
        print('x' * 100)
        print('in az_submit')
        print(session)
        print('x' * 100)
        session_id = session['session.id']
        params = {'session.id': session_id, 'name': job.name, 'description': job.desc}
        az_post(address, az_port, az_timeout, '/manager?action=create', params)
        az_upload('http://' + address + ':' + az_port.__str__() + '/manager', session_id, job.name, job.file)
        flows = json.loads(
            simple_get(
                'http://' + address + ':' + az_port.__str__() + '/manager?session.id=' + session_id +
                '&ajax=fetchprojectflows&project=' + job.name
            )
        )
        response = json.loads(simple_get(
            'http://' + address + ':' + az_port.__str__() + '/executor?session.id=' + session_id +
            '&ajax=executeFlow&project=' + job.name + '&flow=' + flows['flows'][0]['flowId'],
        ))
